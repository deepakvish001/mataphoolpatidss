-- Fixed Final Enhancement: Ensure all users have complete data setup with correct status values

-- First, let's check if we have more courses in the system and add some if needed
DO $$
BEGIN
  -- Add more sample courses if we don't have enough
  IF (SELECT COUNT(*) FROM public.courses WHERE status = 'active') < 5 THEN
  ...
  -- Create assignments for the courses if they don't exist
  IF (SELECT COUNT(*) FROM public.assignments) < 10 THEN
    INSERT INTO public.assignments (course_id, title, description, due_date, status)
    SELECT 
      c.id,
      'Week ' || generate_series(1, 2) || ' Assignment',
      'Complete the practical exercises and submit your project work',
      NOW() + INTERVAL '7 days' * generate_series(1, 2),
      'active'
...
-- Now ensure all existing users have complete data with correct status values
DO $$
DECLARE
  user_record RECORD;
...
BEGIN
  ...
  FOR user_record IN SELECT user_id, full_name FROM public.profiles LOOP
    ...
    -- Create user assignments for enrolled courses using correct status values
